...
export class UserPanelConfession extends Route {
    ...
    async execute(req: any, res: any) {
        ...
        let config = await ConfessionCollection.findOne({ guildId }).catch(() => null);
        if (!config) {
            await ConfessionCollection.insertOne({ guildId, channelId: null });
            config = await ConfessionCollection.findOne({ guildId });
        }
        ...
        const content = await ejs.renderFile(
            path.resolve(__dirname, '../views/confession-config.ejs'),
