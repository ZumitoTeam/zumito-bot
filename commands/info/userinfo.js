...
module.exports = {
	...
	async execute(client, message, args) {
		...
		const embed = new Discord.MessageEmbed()
			...
			.addField(
				'command.userinfo.creation'.trans() + ': ', 
				`<t:${Math.floor(member.user.createdAt / 1000)}:d>` + ' (' + `<t:${Math.floor(member.user.createdAt / 1000)}:R>` + ')')
			.addField(
				'command.userinfo.login'.trans() + ': ', 
				`<t:${Math.floor(member.joinedAt / 1000)}:d>` + ' (' + `<t:${Math.floor(member.joinedAt / 1000)}:R>` + ')')
			.addField(
				'command.userinfo.roles'.trans() + ': ', 
				`${member.roles.cache.filter(r => r.id !== message.guild.id).map(roles => `<@&${roles.id }>`).join(" • ") || 'command.userinfo.noRoles'.trans()}`)
